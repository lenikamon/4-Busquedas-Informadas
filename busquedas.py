...
def busqueda_A_estrella(problema, heuristica):
    ...
    while frontera:
        ...
        for hijo in nodo.expande(problema.modelo):
            costo_actual= hijo.costo + heuristica(hijo)
            if hijo.estado not in visitados or visitados[hijo.estado] > costo_actual:
                visitados[hijo.estado] = costo_actual
                heapq.heappush(frontera, (costo_actual, hijo))
